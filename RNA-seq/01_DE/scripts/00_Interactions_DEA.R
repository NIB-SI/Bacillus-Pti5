...
for(i in 1:length(files)) {
  ...
  colNi <- unlist(strsplit(unlist(strsplit(unlist(strsplit(files[i], ".", fixed = TRUE))[3], "/", fixed = TRUE))[4],"_"))[2]
...
# based on centrifuge results some samples can be discarded here
...
## selecting columns for certain samples!!
...
keep.exprs <- rowSums(y$counts>100)>=11
# keep.exprs <- filterByExpr(y, design, min.count=150, min.total.count=500) #for some reason this filter retains bimodality of densityplots
...
### !!! ###
contrastMatrix = makeContrasts("roots_inoculated_PS_216_YFP-roots_noninoculated",
                               "roots_inoculated_PS_218_YFP-roots_noninoculated",
                               "roots_inoculated_PS_218_MKATE-roots_noninoculated",
                               "roots_inoculated_PS_218_YFP__PS_218_MKATE-roots_noninoculated",
                               "roots_inoculated_PS_216_YFP__PS_218_MKATE-roots_noninoculated",
                               levels=design)
...
tfit
logFCcut <- 1
dt <- decideTests(tfit, lfc=logFCcut, adjust.method = "BH") #defaults adjust.method = "BH", p.value = 0.05, lfc=1
summary(dt)
colnames(dt)
...
## change dt colnames
colnames(dt) <- c("PS216-YFP", "PS218-YFP", "PS218-mKate", "KIN", "NONKIN")

# 3 single strains in Exp2
vennDiagram(dt[, 1:3], include=c("up", "down"), counts.col=c("red", "blue"), circle.col = c("red", "blue", "green3"), cex=c(1,0.8,0.7))
my.vennDiagram(dt[, 1:3], include=c("up", "down"), counts.col=c("red", "blue"), circle.col = c("red", "blue", "green3"), cex=c(1,0.8,0.7))


# KIN and NONKIN interaction
