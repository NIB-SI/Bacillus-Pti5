...
fp = file.path('..', 'input')
fn = 'stu_PGSC-ITAG-merged_2019-05-24_mapping-manual.gmt'
...
fp = file.path('..', 'output', 'comb')
fl = list.files(fp, recursive = TRUE, pattern = '.*\\.tsv')
...
for (i in fl){
  ...
  # name = gsub('S', '', name)
  if (grepl('^leaf', i)) {
    name = paste(stringr::str_split_fixed(name, '_', 3)[1:2], collapse = '_')
  } else {
    name = paste(stringr::str_split_fixed(name, '_', 4)[1:3], collapse = '_')
  }
...
openxlsx::write.xlsx(gmmPlus, file = file.path(fp, fn), asTable = TRUE, overwrite = TRUE)
