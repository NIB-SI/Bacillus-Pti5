...
df = openxlsx::read.xlsx(file.path(fp, fn), sheet = 'Gene expression raw')
...
df = dplyr::left_join(
  df,
  dplyr::filter(df.SE, Genotype == "NT") %>%
    dplyr::select(Gene, NT_mean = Expression_mean),
  by = "Gene"
) %>%
...
perm = perm %>%
  dplyr::left_join(
    df.SE %>% dplyr::select(Gene, Genotype = Genotype, max),
    by = c("Gene", "group2" = "Genotype")
  ) %>%
