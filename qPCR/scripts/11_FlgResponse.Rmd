...
process_data <- function(df, groupvars, measurevar, scale_treatment = "no treatment") {
  ...
  shoots = df.scaled %>% dplyr::filter(Tissue == "leaves")
...
dt$Treatment = factor(trimws(dt$Treatment), levels = c("no treatment", "flg22"))
...
my_dir = file.path("..", "reports", "FlgResponse")
if (!dir.exists(my_dir)) {
  dir.create(my_dir)
}
fr = file.path('..', 'output',  'FlgResponse_stat.txt')
file.create(fr)
...
myorder = c("StRBOHD", "StPR1B", "StBGLU2", "StACO2", "StPti5", "St13-LOX")
...
shoots = process_data(dt, groupvars, measurevar, scale_treatment = "no treatment")$shoots
roots = process_data(dt, groupvars, measurevar, scale_treatment = "no treatment")$roots


cat("FlgResponse", file = fr, append = TRUE, sep = "\n")
cat("", file = fr, append = TRUE, sep = "\n")
...
ggsave(
  filename = file.path(my_dir, "Flg_shoots_1.pdf"),
  plot = result$plot1,
  device = pdf,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "Flg_shoots_1.svg"),
  plot = result$plot1,
  device = "svg",
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "Flg_shoots_2.pdf"),
  plot = result$plot2,
  device = pdf,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "Flg_shoots_2.svg"),
  plot = result$plot2,
  device = svg,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "Flg_roots_1.pdf"),
  plot = result$plot1,
  device = pdf,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "Flg_roots_1.svg"),
  plot = result$plot1,
  device = svg,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "Flg_roots_2.pdf"),
  plot = result$plot2,
  device = pdf,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "Flg_roots_2.svg"),
  plot = result$plot2,
  device = svg,
  path = NULL,
  scale = 1,
