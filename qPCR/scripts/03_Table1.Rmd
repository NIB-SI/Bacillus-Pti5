...
pal = rev(RColorBrewer::brewer.pal(8, 'Paired')[c(3,1,2,7,8)])
...
my_dir = file.path("..", "reports", "Table1")
...
games_howell_by_transcript <- function(mydata.long, measurevar = "measurement", groupvar = "Treatment") {
  ...
  cat("parmutational ANOVA", file = fr, append = TRUE, sep = "\n")
  output_text = capture.output(as.data.frame(pA))
  ...
  resid_qq_list = mydata.long %>%
    ...
    dplyr::group_map(~ {
      ...
      ggpubr::ggqqplot(resids) +
        ggplot2::ggtitle(paste("Resid:", .y$transcript))
    })
...
test_and_plot <- function(data_long_raw
  ...
  # output_text = capture.output(as.data.frame(results$distr_measurement))
  ...
  output_text = apply(results$wilcoxon_measurement, 1, function(row) paste(row, collapse = "\t")) # do  not split in multiple lines
  cat(output_text, file = fr, append = TRUE, sep = "\n")
...
df[, Sample.ID := NULL]
df[, Genotype := NULL]
df$Treatment = factor(df$Group, levels = c("noninoculated", "PS-216", "PS-216 medium", "PS-216 srfA", "PS-216 srfA medium"))
df[, Strain := NULL]
df[, Group := NULL]
...
myorder = c("RBOHD", "CPI8", "CAB", "BGLU2", "HSP70", "PTI5", "13-LOX")
group1_names = c("PTI5", "13-LOX")

make_scale_params <- function(maxval) {
  upper = ifelse(maxval > 2, maxval + 2, maxval + 0.1)
...
build_y_scales_for <- function(names_vec) {
  max_per_transcript %>%
...
y_scales1 = build_y_scales_for(group1_names)
y_scales2 = build_y_scales_for(group2_names)
...
cat("", file = fr, append = TRUE, sep = "\n")
output_text = capture.output(paste0("Games Howell"))
cat(output_text, file = fr, append = TRUE, sep = "\n")
output_text = capture.output(print(res, n = Inf))
cat(output_text, file = fr, append = TRUE, sep = "\n")
cat("", file = fr, append = TRUE, sep = "\n")
