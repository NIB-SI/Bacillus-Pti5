...
my_dir = file.path("..", "reports", "AMFresponse")
if (!dir.exists(my_dir)) {
  dir.create(my_dir)
}

fr = file.path('..', 'output',  'AMFresponse_stat.txt')
file.create(fr)
...
process_data <- function(df, groupvars, measurevar, scale_treatment = "noninoculated") {
  ...
  df.long = df %>%
    tidyr::pivot_longer(cols = 5:ncol(df), names_to = "transcript",
...
sheet_name = grep("AMFresponse", sheet_names, value = TRUE)
...
myorder = c("StRBOHD", "StPR1B", "StCPI8", "StBGLU2", "StHSP70", "StPti5", "St13-LOX")
...
ggsave(
  filename = file.path(my_dir, "AMF-addOn_Rywal_1.svg"),
  plot = result$plot1,
  device = svg,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "AMF-addOn_Rywal_2.svg"),
  plot = result$plot2,
  device = svg,
  path = NULL,
  scale = 1,
