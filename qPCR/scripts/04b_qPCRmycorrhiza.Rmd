---
title: "qPCR mycorrhiza - additional genes"
author: "zagor"
date: "`r Sys.Date()`"
output:
  html_document:
...
col = RColorBrewer::brewer.pal(8, 'Paired')[c(3,4, 5,6, 7, 8)]

my_dir = file.path("..", "reports", "qPCR_mycorrhiza")
if (!dir.exists(my_dir)) {
  dir.create(my_dir)
}

fr = file.path('..', 'output',  'qPCRmycorrhiza_EMM_stat.txt')
file.create(fr)
...
process_data <- function(df, groupvars, measurevar, scale_treatment = "noninoculated") {
  ...
  scale_reference = data.SE %>%
    ...
    dplyr::select(Genotype, transcript, mean) %>%
    dplyr::rename(scale_mean = mean)
  df.scaled <- dplyr::left_join(df.long, scale_reference, by = c("Genotype", "transcript")) %>%
...
## Generalized Linear Model, with Estimated marginal means followed by pairwise comparisons

- fit a Gamma GLM on the subset for that transcript; model coefficients are on the log scale

- compute heteroscedasticity‑consistent (HC3) covariance for the GLM coefficients

- compute estimated marginal means using the robust vcov so SEs reflect heteroscedasticity

- pairwise comparisons with the multivariate‑t simultaneous adjustment that uses the supplied covariance

- residual diagnostics and plotting: simulate standardized residuals for the model, produce diagnostic output

- FDR‑adjusted p‑values across all contrasts from all transcrip in this case are not feasible

  - mixing two different multiple‑testing corrections and produces results that are hard to interpret and usually overly conservative
  - BH algorithm assumes p‑values are approximately uniform under the null, an assumption broken by prior single‑step adjustmet
...
emmeans_by_transcript <- function(mydata.long, measurevar = "measurement", groupvar = c("Treatment", "Genotype")) {
  ...
  temp = mydata.long %>%
    ...
    dplyr::group_map(~ {
      # Fit Gamma GLM for each transcript
      model = glm(measurement ~ Genotype * Treatment, data = .x, family = Gamma(link = "log"))
      # Heteroscedasticity-Consistent Covariance Matrix Estimation
      vc = sandwich::vcovHC(model, type = "HC3")
      # Estimated marginal means
      emm = emmeans::emmeans(model, ~ Genotype * Treatment, vcov. = vc)
      # Pairwise comparisons
      temp = pairs(emm, adjust = "mvt")
      result = summary(temp) %>%
        dplyr::mutate(transcript = .y$transcript)
      # diagnostics
      # qqnorm(residuals(model), main = paste("Q-Q Plot:", .y$transcript))
      # qqline(residuals(model), col = "darkblue", lwd = 2)
      sim_res = DHARMa::simulateResiduals(model)
      par(mfrow = c(2, 2), oma = c(0, 0, 3, 0))  # Bottom, left, top, right
      # Plot the 4 diagnostics
      DHARMa::plotResiduals(sim_res, form = model$Genotype)
      DHARMa::plotResiduals(sim_res, form = model$Treatment)
      DHARMa::testUniformity(sim_res)
      DHARMa::testOutliers(sim_res)
      # Add shared title across all plots
      mtext(paste("DHARMA diagnostics:", .y$transcript), side = 3, line = 1, outer = TRUE, cex = 1.2, font = 2)
      # Reset layout
      par(mfrow = c(1, 1))
      # plot emmeans per transcript
      print(plot(emm, comparison = TRUE, adjust = "mvt"))
      return(result)
    }) %>%
    dplyr::bind_rows() %>%
    dplyr::mutate(
...
plot_gene_expression <- function(  data.SE
  ...
  data.SE.filtered = data.SE.filtered %>%
    dplyr::mutate(Group = paste(Genotype, Treatment, sep = "\n"))
  
  data.long.filtered = data.long.filtered %>%
    dplyr::mutate(Group = paste(Genotype, Treatment, sep = "\n"))
  ...
  group_levels = c(
    "Rywal\nnoninoculated", "Rywal\ninoculated",
    "shPTI5-Rywal L2\nnoninoculated", "shPTI5-Rywal L2\ninoculated",
    "shPTI5-Rywal L6\nnoninoculated", "shPTI5-Rywal L6\ninoculated"
  )
  
  # Apply factor levels
  data.SE.filtered$Group = factor(data.SE.filtered$Group, levels = group_levels)
  data.long.filtered$Group = factor(data.long.filtered$Group, levels = group_levels)
  
  stat.test.filtered = stat.test.filtered %>%
    dplyr::mutate(
      xmin = match(group1, sub('\\\n', ' ', group_levels)),
      xmax = match(group2, sub('\\\n', ' ', group_levels))
    )

  p = ggplot(data.SE.filtered, aes(x = Group, y = mean)) +
    geom_point(size = 3.5, shape = 22, position = dodge, aes(fill = Group), colour = "black") +
    # geom_point(data = data.long.filtered, aes(x = Group, y = scaled, fill = Group), colour = "black",
    #            size = 2.0, shape = 21, position = dodge) +
    geom_point(data = data.long.filtered, aes(x = Group, y = measurement, fill = Group), colour = "black",
               size = 2.0, shape = 21, position = dodge) +
    geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.3, lwd = 0.5, position = dodge) +
    facet_wrap(~ transcript, ncol = facet_cols, scales = "free", drop = TRUE)

  p = p + geom_hline(yintercept = 0, alpha = 0.0) +
...
dens_and_effect <- function(mydata.long, p_colors) {
  ...
  p1 = ggpubr::ggdensity(mydata.long, x = "measurement",
    ...
    ggtitle("measurement") +
    facet_wrap(Genotype~transcript, ncol = 7, scales = "free")
  print(p1)
  
  cat(crayon::red('####  ####  \nDistribution tests\n####  ####  \n'))
  
    distr_tests = mydata.long %>%
    dplyr::group_by(Genotype, transcript) %>%
    dplyr::group_map(~ {
  ...
  combined_results = distr_tests[grep("^n$|transcript|Genotype|_BH", colnames(distr_tests))]
  ...
  resid_density_list = mydata.long %>%
    ...
    dplyr::group_map(~ {
      ...
      ggplot(df, aes(x = resids)) +
        ...
        ggtitle(paste("", .y$transcript)) +
        ggplot2::ylab(paste("", .y$Genotype)) +
        theme_minimal()
    })
  
  # Arrange and print all plots
  qq_arranged = ggpubr::ggarrange(plotlist = qq_plot_list, ncol = 7, nrow = ceiling(length(qq_plot_list) / 7))
  resid_qq_arranged = ggpubr::ggarrange(plotlist = resid_qq_list, ncol = 7, nrow = ceiling(length(resid_qq_list) / 7))
  resid_hist_arranged = ggpubr::ggarrange(plotlist = resid_density_list, ncol = 7, nrow = ceiling(length(resid_density_list) / 7))
...
test_and_plot <- function(data_long_raw
  ...
  mydata.long.SE = summary_stats(mydata.long, measurevar = "measurement", groupvars = groupvars)
  ...
  temp = emmeans_by_transcript(mydata.long, measurevar = "measurement", groupvar = c("Treatment", "Genotype"))
  temp$group1 = trimws(gsub("[()]", "", sub(" -.*", "", temp$contrast)))
  temp$group2 = trimws(gsub("[()]", "", sub(".*- ", "", temp$contrast)))
  stat.test = temp %>%
    dplyr::select(transcript, group1, group2, p.adj, p.adj.signif)
  

  
    # Step 1: Create combined group labels
  mydata.long = mydata.long %>%
    dplyr::mutate(Group = paste(Genotype, Treatment))
  
  # Step 2: Compute y-axis range for each transcript
    ...
    dplyr::left_join(
      mydata.long %>%
        ...
        dplyr::summarise(
          ...
          max_scaled = max(measurement, na.rm = TRUE),
          min_scaled = min(measurement, na.rm = TRUE),
          .groups = "drop"
        ),
      by = "transcript"
    ) %>%
  ...
  # Step 3: Map group labels to numeric x-axis positions
  pos_map = mydata.long %>%
    dplyr::distinct(transcript, Group) %>%
    dplyr::mutate(xpos = as.numeric(factor(Group)))
  
  # Step 4: Join positions to stat.test
  stat.test = stat.test %>%
    dplyr::left_join(y_pos_df, by = "transcript") %>%
    dplyr::left_join(pos_map %>% dplyr::rename(group1 = Group, xmin = xpos), by = c("transcript", "group1")) %>%
    dplyr::left_join(pos_map %>% dplyr::rename(group2 = Group, xmax = xpos), by = c("transcript", "group2")) %>%
    dplyr::mutate(
      xmin = dplyr::if_else(is.na(xmin), as.numeric(factor(group1)), xmin),
      xmax = dplyr::if_else(is.na(xmax), as.numeric(factor(group2)), xmax)
    )
  
  # Step 5: Compute y.position for significant comparisons
...
data.table::setDT(df)
df[, Sample.name := NULL]
df$Treatment = factor(df$Treatment, levels = c("noninoculated", "inoculated"))
...
groupvars = c("Genotype", "Treatment", "transcript")
measurevar = 'measurement'
dodge = position_dodge(width = 0.5)

myorder = c("RBOHD", "PR1B", "CPI8", "BGLU2", "HSP70", "PTI5", "13-LOX")
...
amf = process_data(df, groupvars, measurevar, scale_treatment = "noninoculated")$amf
...
max_per_transcript = amf %>%
  dplyr::group_by(transcript) %>%
  # dplyr::summarise(max_scaled = max(scaled, na.rm = TRUE), .groups = "drop")
  dplyr::summarise(max_scaled = max(measurement, na.rm = TRUE), .groups = "drop")
...
result = test_and_plot(data_long_raw = amf
                       , myorder = myorder
                       , pal = p
                       , what = "AMF"
                       , plot_gene_expression_func = plot_gene_expression
                       , groupvars = c("Treatment", "Genotype", "transcript")
                       , y_scales1
                       , y_scales2
                       )
...
ggsave(
  filename = file.path(my_dir, "AMF-addOn_1.pdf"),
  plot = result$plot1,
  device = pdf,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "AMF-addOn_2.pdf"),
  plot = result$plot2,
  device = pdf,
  path = NULL,
  scale = 1,
