...
my_dir = file.path("..", "reports", "BsMutants")
if (!dir.exists(my_dir)) {
  dir.create(my_dir)
}
fr = file.path('..', 'output',  'BsMutants_stat.txt')
file.create(fr)
...
sheet_name = grep("BsMutants", sheet_names, value = TRUE)
...
group1_names = c("StCAB", "StRBOHD", "StPti5", "St13-LOX")
...
ggsave(
  filename = file.path(my_dir, "shoots_1.svg"),
  plot = result$plot1,
  device = svg,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "shoots_2.svg"),
  plot = result$plot2,
  device = svg,
  path = NULL,
  scale = 1,
