...
# pal = col[seq(1, n, 2)]
pal = rev(RColorBrewer::brewer.pal(8, 'Paired')[c(3,7)])
...
process_data <- function(df, groupvars, measurevar, scale_Strain = "PS-216") {
  ...
  scale_reference = data.SE %>%
    dplyr::filter(Strain == scale_Strain) %>%
  ...
  df.scaled = df.scaled %>% 
    dplyr::arrange(dplyr::desc(transcript), dplyr::desc(Strain))
  shoots = df.scaled %>% dplyr::filter(Tissue == "lower stem with lower leaves")
...
perm_test_by_transcript <- function(mydata.long, measurevar = "measurement", groupvar = "Strain") {
  
  set.seed(123456)
  
  temp = data.frame()
  transcript_levels = levels(mydata.long$transcript)
  Strain_levels = levels(mydata.long[[groupvar]])
  Strain_pairs = combn(Strain_levels, 2, simplify = FALSE)
  
  for(i in transcript_levels) {
    data = mydata.long[mydata.long$transcript == i, ]
    k = 12 # nrow(data)
    pvalues = purrr::map_dbl(Strain_pairs, function(x) {
    ...
    colnames(tmp) = purrr::map_chr(Strain_pairs, ~base::paste(.x, collapse = ' vs '))
    rownames(tmp) = i
...
plot_gene_expression <- function(data.SE
                                 , data.long
                                 , stat.test.sig
                                 , transcripts_excl = c('St13-LOX', 'StPti5')
                                 , color_values
                                 , facet_cols = 6
                                 , y_scales = NULL
                                 , dodge_width = 0.8,
  ...
  p = ggplot(data.SE.filtered, aes(x = Strain, y = mean)) +
    geom_point(size=3.5, shape = 22, position = dodge, aes(fill = Strain), colour = "black") +
    geom_point(data = data.long.filtered, aes(x = Strain, y = scaled, fill = Strain), colour = "black",
               size = 2.0, shape = 21,
               position = dodge) +
...
dens_and_effect <- function(mydata.long, p_colors) {
  ...
  resid_qq_list = mydata.long %>%
    dplyr::group_by(transcript) %>%
    dplyr::group_map(~ {
      model = lm(measurement ~ Strain, data = .x)
      resids = resid(model)
  ...
  lev_meas = mydata.long %>%
    dplyr::group_by(transcript) %>%
    rstatix::levene_test(measurement ~ Strain, center = "mean")
  print(lev_meas)
  ...
  bf_meas = mydata.long %>%
    dplyr::group_by(transcript) %>%
    rstatix::levene_test(measurement ~ Strain, center = "median")
  print(bf_meas)
  
  cat(crayon::red("####  ####  \nFligner\n####  ####  \n"))
  fk_meas = mydata.long %>%
    dplyr::group_by(transcript) %>%
    dplyr::group_modify(~ broom::tidy(stats::fligner.test(measurement ~ Strain, data = .x)))
  print(fk_meas)
  ...
  eff_meas = mydata.long %>%
    dplyr::group_by(transcript) %>%
    rstatix::wilcox_effsize(measurement ~ Strain)
  print(eff_meas)
  
  # Wilcoxon effect size on scaled
  # eff_scaled = mydata.long %>%
  #   dplyr::group_by(transcript) %>%
  #   rstatix::wilcox_effsize(scaled ~ Strain)
  # print(eff_scaled)
  ...
  coh_meas = mydata.long %>%
    dplyr::group_by(transcript) %>%
    rstatix::cohens_d(measurement ~ Strain, paired = FALSE)
  print(coh_meas)
...
test_and_plot <- function(data_long_raw
  ...
  temp = perm_test_by_transcript(mydata.long, measurevar = "measurement", groupvar = "Strain")
  ...
  pos_map = mydata.long %>%
    ...
    dplyr::mutate(xpos = as.numeric(factor(Strain, levels = levels(mydata.long$Strain))))
  
  # Attach y-position and x-position info to stat.test
  stat.test = stat.test %>%
    dplyr::left_join(y_pos_df, by = "transcript") %>%
    dplyr::left_join(pos_map %>% dplyr::rename(group1 = Strain, xmin = xpos), by = c("transcript", "group1")) %>%
    dplyr::left_join(pos_map %>% dplyr::rename(group2 = Strain, xmax = xpos), by = c("transcript", "group2")) %>%
    dplyr::mutate(
      xmin = dplyr::if_else(is.na(xmin), as.numeric(factor(group1, levels = levels(mydata.long$Strain))), xmin),
      xmax = dplyr::if_else(is.na(xmax), as.numeric(factor(group2, levels = levels(mydata.long$Strain))), xmax)
    )
...
sheet_name = grep("BsMutantAbundance", sheet_names, value = TRUE)

dt = openxlsx::read.xlsx(xlsxFile = file.path(fp, fn),
                            sheet = sheet_name,
                            startRow = 10, 
                            colNames = TRUE,
                            rowNames = FALSE,
...
dt[, Sample.ID := NULL]
dt[, Genotype := NULL]
dt$Tissue = as.factor(trimws(dt$Tissue))
dt$Strain = factor(trimws(dt$Strain), levels = c("PS-216", "PS-216 srfA"))
...
my_dir = file.path("..", "reports", "BsMutantAbundance")
if (!dir.exists(my_dir)) {
  dir.create(my_dir)
}
fr = file.path('..', 'output',  'BsMutantAbundance_stat.txt')
file.create(fr)
...
groupvars = c("Tissue", "Strain", "transcript")
measurevar = 'measurement'
dodge = position_dodge(width = 0.5)

myorder = c("BsGyrB")
group1_names = c("BsGyrB")
...
shoots = process_data(dt, groupvars, measurevar, scale_Strain = "PS-216")$shoots
roots = process_data(dt, groupvars, measurevar, scale_Strain = "PS-216")$roots


cat("BsMutantAbundance", file = fr, append = TRUE, sep = "\n")
cat("", file = fr, append = TRUE, sep = "\n")
...
ggsave(
  filename = file.path(my_dir, "BsMutantAbundance_shoots_1.pdf"),
  plot = result$plot1,
  device = pdf,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "BsMutantAbundance_shoots_1.svg"),
  plot = result$plot1,
  device = "svg",
  path = NULL,
  scale = 1,
...
if (length(y_scales2) > 0) {
  ggsave(
    filename = file.path(my_dir, "BsMutantAbundance_shoots_2.pdf"),
    plot = result$plot2,
    device = pdf,
    path = NULL,
    scale = 1,
  ...
  ggsave(
    filename = file.path(my_dir, "BsMutantAbundance_shoots_2.svg"),
    plot = result$plot2,
    device = svg,
    path = NULL,
    scale = 1,
...
ggsave(
  filename = file.path(my_dir, "BsMutantAbundance_roots_1.pdf"),
  plot = result$plot1,
  device = pdf,
  path = NULL,
  scale = 1,
...
ggsave(
  filename = file.path(my_dir, "BsMutantAbundance_roots_1.svg"),
  plot = result$plot1,
  device = svg,
  path = NULL,
  scale = 1,
...
if (length(y_scales2) > 0) {
  ggsave(
    filename = file.path(my_dir, "BsMutantAbundance_roots_2.pdf"),
    plot = result$plot2,
    device = pdf,
    path = NULL,
    scale = 1,
  ...
  ggsave(
    filename = file.path(my_dir, "BsMutantAbundance_roots_2.svg"),
    plot = result$plot2,
    device = svg,
    path = NULL,
    scale = 1,
